...
function buildA4Html(detail: DeliveryPreview): string {
  const lineRows =
    detail.lines.length === 0
      ? `<tr><td colspan="4" class="empty">No delivery lines.</td></tr>`
      : detail.lines
          .map(
            (line, index) => `
        <tr>
          ...
          <td class="num">${formatQty(line.demand)} ${escapeHtml(line.unit || 'Units')}</td>
          <td class="num strong">${formatQty(line.quantity)} ${escapeHtml(line.unit || 'Units')}</td>
        </tr>`,
          )
          .join('');
...
<body>
  <div class="page">
    <div class="top">
      ...
      <div class="doc-box">
        <div class="doc-badge">DELIVERY</div>
        <div class="doc-number">${escapeHtml(detail.name)}</div>
        <div class="doc-date">${formatDateTime(detail.scheduledDate || detail.effectiveDate)}</div>
      </div>
    </div>
    ...
    <div class="info-grid">
      ...
      <div class="info-block">
        ...
        <p><span class="info-label">SCHEDULED:</span> <strong>${formatDateTime(detail.scheduledDate)}</strong></p>
        <p><span class="info-label">EFFECTIVE:</span> <strong>${formatDateTime(detail.effectiveDate)}</strong></p>
      </div>
    </div>
    ...
    <table class="lines">
      <thead>
        <tr>
          ...
          <th>PRODUCT</th>
          <th class="num" style="width:110px">DEMAND</th>
          <th class="num" style="width:110px">DONE</th>
        </tr>
      </thead>
...
function buildThermalHtml(detail: DeliveryPreview): string {
  const lineRows =
    detail.lines.length === 0
      ? `<tr><td colspan="2" class="muted">No lines</td></tr>`
      : detail.lines
          .map(
            line => `
        <tr>
          <td class="product">${escapeHtml(line.product)}</td>
          <td class="qty-col">${formatQtyWhole(line.quantity)} ${escapeHtml(line.unit || '')}</td>
        </tr>`,
          )
          .join('');
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    table.lines td.product { word-break: break-word; width: 70%; }
    table.lines .qty-col { text-align: right; white-space: nowrap; width: 30%; }
    .footer {
      text-align: center;
      font-size: 12px;
      font-weight: 700;
...
<body>
  <div class="receipt">
    <div class="center">
      ...
      <div class="meta">DELIVERY # ${escapeHtml(detail.name)} · ${formatDate(detail.scheduledDate || detail.effectiveDate)}</div>
    </div>
    <hr class="divider" />
    ...
    <div class="customer-line"><strong>ORIGIN:</strong> <strong>${displayValue(detail.origin)}</strong></div>
    <hr class="divider" />
    <table class="lines">
      <thead>
        <tr>
...
export function buildDeliveryPrintHtml(
