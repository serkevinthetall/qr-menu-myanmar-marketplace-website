...
import { SaleOrderDetail } from '@/types/sale-order';
import {
  formatMyanmarDate,
...
export type SaleOrderPrintLabel = 'SALE ORDER' | 'APP ORDER';
...
function buildA4Html(
  ...
  documentLabel: SaleOrderPrintLabel,
): string {
  const untaxed =
...
<body>
  <div class="page">
    <div class="top">
      ...
      <div class="doc-box">
        <div class="doc-badge">${escapeHtml(documentLabel)}</div>
    ...
    <div class="bottom">
      <div>
        <div class="note-box">
          ...
          <div>${displayValue(detail.deliveryAddress)}</div>
        </div>
      </div>
      <div class="totals">
...
function buildThermalHtml(
  ...
  const shortLabel = documentLabel === 'APP ORDER' ? 'APP' : 'SO';
  const lineRows =
    detail.lines.length === 0
...
<body>
  <div class="receipt">
    <div class="center">
      ...
      <div class="meta">${escapeHtml(shortLabel)} # ${escapeHtml(detail.number)} · ${formatDate(detail.orderDate)}</div>
    </div>
    ...
    <div class="field">
      <strong>STATUS:</strong> ${displayValue(detail.status)}
    </div>
...
/** Builds printable HTML for a sale / app order. */
export function buildSaleOrderPrintHtml(
  detail: SaleOrderDetail,
  format: PrintFormat,
  documentLabel: SaleOrderPrintLabel = 'SALE ORDER',
): string {
  return format === 'a4'
    ? buildA4Html(detail, documentLabel)
    : buildThermalHtml(detail, documentLabel);
}
