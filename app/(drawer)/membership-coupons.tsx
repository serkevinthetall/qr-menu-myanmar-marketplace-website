...
export default function MembershipCouponsScreen() {
  ...
  const query = useModuleSearch('Search coupon tickets');
    ...
    <View style={[styles.root, { backgroundColor: theme.colors.background }]}>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No coupon tickets found.</Text>
          }
          renderItem={({ item }) => (
            <Pressable
              ...
              <Text style={{ color: theme.colors.onSurfaceVariant, marginTop: 4 }}>
                ...
                {item.ticketMonth ? ` · ${item.ticketMonth}` : ''}
              </Text>
            </Pressable>
          )}
        />
